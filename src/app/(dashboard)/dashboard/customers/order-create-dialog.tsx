      ...
      <DialogContent className="sm:max-w-[650px]">
          ...
          <div className="space-y-4">
            ...
            <div className="p-5 bg-slate-900 text-white rounded-2xl space-y-4 shadow-lg">
              ...
              <div className="space-y-3">
                <div className="space-y-1.5">
                   <div className="flex justify-between items-center">
                     <Label className="text-[11px] opacity-80 flex items-center gap-1"><Banknote className="w-3 h-3" /> Số tiền giảm (VNĐ)</Label>
                   </div>
                   <Input 
                        type="number" 
                        placeholder="0"
                        ...
                        className="bg-slate-800 border-none h-9 text-sm text-yellow-400 font-bold focus-visible:ring-yellow-500/50"
                    />
                </div>

                <div className="space-y-1.5">
                   <Label className="text-[11px] opacity-80 flex items-center gap-1"><Percent className="w-3 h-3" /> Phần trăm giảm (%)</Label>
                   <Input 
                        type="number" 
                        placeholder="0"
                        min="0"
              ...
              <div className="flex justify-between items-center pt-3 border-t border-white/10 mt-2">
                <span className="font-medium text-sm">Giá cuối còn lại:</span>
                <span className="text-2xl font-black text-emerald-400">{formatCurrency(finalPrice)}</span>
              </div>
            </div>

            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
                <Label htmlFor="pay-amt" className="text-xs font-bold text-slate-700">Số tiền cần thanh toán ngay (Để sinh QR)</Label>
                <div className="relative">
                    <Input 
                        id="pay-amt" 
                        type="number" 
                        placeholder="Mặc định theo Giá cuối"
                        className="h-9 pr-10"
                        value={paymentAmount || ''}
                ...
                <p className="text-[10px] text-slate-500 italic leading-tight">
                    * QR sẽ sinh theo số tiền này. Nếu để trống hoặc 0, mã QR sẽ tự động lấy theo <strong>{formatCurrency(finalPrice)}</strong>.
                </p>
            </div>
          </div>
        </div>
        ...
        <DialogFooter className="bg-slate-50 -mx-6 -mb-6 p-6 rounded-b-lg border-t gap-2 sm:gap-0">
          ...
          <div className="flex gap-2">
            <Button variant="secondary" onClick={handleCreateOrder} disabled={loading || selectedCourseIds.length === 0}>
                {loading ? <Loader2 className="animate-spin h-4 w-4" /> : 'Chỉ lưu đơn'}
            </Button>
            <Button onClick={handleCreateOrder} disabled={loading || selectedCourseIds.length === 0} className="min-w-[150px] shadow-md shadow-primary/20">
